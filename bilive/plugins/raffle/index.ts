...
class Raffle extends Plugin {
  ...
  public async load({ defaultOptions, whiteList }: { defaultOptions: options, whiteList: Set<string> }) {
    ...
    defaultOptions.config['raffleDrop'] = 0
    ...
    // beatStorm丢弃概率
    defaultOptions.config['beatStormDrop'] = 0
    ...
    // beatStorm类抽奖
    defaultOptions.newUserData['webBeatStorm'] = false
    defaultOptions.info['webBeatStorm'] = {
      description: 'Web节奏风暴',
      tip: '自动参与Web节奏风暴',
      type: 'boolean'
    }
    whiteList.add('webBeatStorm')
    // beatStorm类抽奖
    defaultOptions.newUserData['appBeatStorm'] = false
    defaultOptions.info['appBeatStorm'] = {
      description: 'APP节奏风暴',
      tip: '自动参与APP节奏风暴，未实名可用',
      type: 'boolean'
    }
    whiteList.add('appBeatStorm')
    // beatStorm限制
    ...
    // beatStorm刷新时间
    defaultOptions.config['beatStormRefresh'] = Date.now()
  ...
  private async _loadCount(options: options, users: Map<string, User>) {
    ...
    if (Date.now() - <number>options.config['beatStormRefresh'] < todaySecondsPassed * 1000)
  ...
  public async loop({ cstMin, cstHour, cstString, options, users }: { cstMin: number, cstHour: number, cstString: string, options: options, users: Map<string, User> }) {
    ...
    options.config['beatStormRefresh'] = Date.now()
    Options.save()
  }
  ...
  private _Lottery() {
    ...
    if (queue !== undefined) {
      // @ts-ignore
      if (tools.ban[`${Options._.config[`${queue.message.cmd}API`]}/join`]) {
  ...
  private async _doRaffle({ message, options, users }: { message: raffleMessage | lotteryMessage, options: options, users: Map<string, User> }) {
    ...
    for (let [uid, user] of users) {
      if (user.validateURL !== '' || !user.userData[message.cmd]) continue
      if (this._raffleBanList.get(uid)) continue
      const droprate = <number>options.config['raffleDrop']
  ...
  private async _doStorm({ message, options, users }: { message: beatStormMessage, options: options, users: Map<string, User> }) {
    users.forEach(async (user, uid) => {
      ...
      if (user.validateURL !== '' || (!user.userData['appBeatStorm'] && !user.userData['webBeatStorm'])) return
      ...
      const droprate = <number>options.config['beatStormDrop']
