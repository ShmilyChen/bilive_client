...
class Raffle extends EventEmitter {
  ...
  private async _Lottery() {
    ...
    const reward: XHRoptions = {
      method: 'POST',
      url: `https://api.live.bilibili.com/xlive/lottery-interface/v2/Lottery/join`,
  ...
  private async _BeatStorm() {
    ...
    const { 0: wait, 1: limt } = <Array<number>>Options._.config.stormSetting
    if (this._user.userData['webBeatStorm']) this._doWebStorm(wait, limt)
    if (this._user.userData['appBeatStorm']) this._doAppStorm(wait, limt)
  }
  ...
  private async _doWebStorm(wait: number, limt: number) {
    ...
    for (let i = 1; i <= limt; i++) {
      ...
      // await tools.Sleep(start+(<number[]>Options._.config.stormSetting)[0] - Date.now())
