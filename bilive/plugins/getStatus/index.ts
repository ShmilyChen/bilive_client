...
class GetStatus extends Plugin {
  ...
  private async _getStatus(users: Map<string, User>, push: boolean) {
    ...
    for (const [uid, user] of users) {
      ...
      tmp['judgementData'] = await this._geJudgement(user)
  ...
  private async _getLiveInfo(user: User) {
    const userInfo: XHRoptions = {
      url: `https://api.live.bilibili.com/User/getUserInfo?ts=${AppClient.TS}`,
      responseType: 'json',
      cookieJar: user.jar,
   ...
   * @memberof GetStatus
   */
  private async _geJudgement(user: User) {
    ...
    if (caseList !== undefined && caseList.response.statusCode === 200 && caseList.body.code === 0)
      return {
        success: caseList.body.data.filter(data => new Date(data.voteTime).toString().slice(0, 10) === new Date().toString().slice(0, 10) && data.vote !== 0).length,
        fail: caseList.body.data.filter(data => new Date(data.voteTime).toString().slice(0, 10) === new Date().toString().slice(0, 10) && data.vote === 0).length
      }
    else return {
      success: 0,
      fail: 0
...
银瓜子：${user.liveData.silver}  硬币：${user.liveData.billCoin}  当前状态：${ban} \n今日风纪案件：完成${user.judgementData.success}个，未投票${user.judgementData.fail}个 `)
        }
      }()
      let medalDiv: string = '\n\n-------------------------- 佩戴勋章信息 --------------------------\n'
      medal = function () {
